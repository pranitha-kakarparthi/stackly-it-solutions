...
document.querySelector('#prevTestimonial').addEventListener('click', () => showSlide(slideIndex - 1));
document.querySelector('#nextTestimonial').addEventListener('click', () => showSlide(slideIndex + 1));
...
if (description) description.setAttribute('content', 'Stackly builds intelligent digital products, cloud systems, and secure technology solutions for ambitious businesses.');
...
if (socialLinks) {
  ...
  ].map(([label, icon]) => `<a href="404.html" aria-label="${label}"><i class="fa-brands ${icon}"></i></a>`).join('');
}
